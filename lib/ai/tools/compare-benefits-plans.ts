...
const compareBenefitsPlansSchema = z.object({
  ...
  comparisonType: z.enum(['cost', 'coverage', 'features', 'all']).default('all').describe('Type of comparison to perform'),
  userContext: z.object({
    coverageType: z.enum(['individual', 'family']).optional().describe('Coverage type for cost calculations'),
    priorities: z.array(z.string()).optional().describe('User priorities (e.g., "low cost", "comprehensive coverage")')
  }).optional().describe('User-specific context for personalized comparison')
});

export const compareBenefitsPlans = tool({
  description: 'Compare multiple benefit plans side by side with detailed analysis of costs, coverage, and features',
  parameters: compareBenefitsPlansSchema,
  execute: async (rawParams) => {
    const { planIds, comparisonType, userContext } = compareBenefitsPlansSchema.parse(rawParams);
    ...
    try {
      ...
      try {
        ...
        const analysis = {
          ...
          coverageType,
          costComparison: {
            lowestMonthlyCost: Math.min(...plansWithAnalysis.map(p => p.costs.monthlyCost)),
            highestMonthlyCost: Math.max(...plansWithAnalysis.map(p => p.costs.monthlyCost)),
            averageDeductible: plansWithAnalysis.reduce((sum, p) => sum + p.costs.deductible, 0) / plansWithAnalysis.length,
            lowestDeductible: Math.min(...plansWithAnalysis.map(p => p.costs.deductible)),
            highestDeductible: Math.max(...plansWithAnalysis.map(p => p.costs.deductible))
          },
          recommendations: generateRecommendations(plansWithAnalysis, userContext?.priorities || [])
        };
