...
import { BenefitPlan } from '@/lib/types/benefit-plan.type';
...
const checkEligibilitySchema = z.object({
  benefitType: z.enum(['health', 'dental', 'vision', 'life', 'disability', 'fsa', 'hsa', '401k']),
  employeeId: z.string(),
  eventType: z.enum(['new_hire', 'open_enrollment', 'qualifying_event']).optional(),
...
const getEnrollmentDeadlineSchema = z.object({
  ...
  enrollmentPeriod: z.enum(['annual', 'new_hire', 'qualifying_event']).optional(),
});
...
export const calculateCost = tool({
  ...
  execute: async ({ planId, annualSalary, coverageLevel, includeHSA, includeFSA, estimatedMedicalExpenses }: z.infer<typeof calculateCostSchema>) => {
      ...
      const totalOutOfPocket = employeeCost - taxSavings + (estimatedMedicalExpenses || 0);
      const totalWithSavingsAccounts = totalOutOfPocket - hsaTaxSavings - fsaTaxSavings;
...
export const checkEligibility = tool({
  ...
  execute: async ({ benefitType, employeeId, eventType, eventDate }: z.infer<typeof checkEligibilitySchema>) => {
      ...
      return {
        ...
        nextEnrollmentWindow: eventType === 'qualifying_event'
...
export const getEnrollmentDeadline = tool({
  ...
  execute: async ({ companyId, benefitType, enrollmentPeriod }: z.infer<typeof getEnrollmentDeadlineSchema>) => {
      ...
      if (enrollmentPeriod === 'new_hire' || enrollmentPeriod === 'qualifying_event') {
        deadline = new Date(today);
        deadline.setDate(deadline.getDate() + 30);
      } else { // Annual open enrollment
...
export const explainBenefit = tool({
  ...
  execute: async ({ term, context }: z.infer<typeof explainBenefitSchema>) => {
      ...
      const definitions: Record<string, string> = {
        ...
        'out-of-pocket maximum': 'The most you have to pay for covered services in a plan year. After you reach this amount, your health plan pays 100% of covered services.',
      ...
      const definition = definitions[termLower] || `"${term}" is a benefits-related term. Please ask HR for more specific information.`;
