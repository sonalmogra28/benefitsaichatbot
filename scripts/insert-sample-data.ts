...
async function insertSampleData() {
  ...
  try {
    ...
    let company = await db.query.companies.findFirst({
      where: (companies, { eq }) => eq(companies.stackOrgId, 'default-org-sample')
    });
    ...
    const healthPlans = await db.insert(benefitPlans).values([
      {
        companyId: company.id,
        ...
        features: ['Nationwide network', 'No referrals needed', 'Prescription coverage'],
      ...
      },
      {
        ...
        features: ['Lower premiums', 'Coordinated care', 'Preventive care covered'],
        effectiveDate: '2024-01-01',
        isActive: true,
      },
      {
    ...
    const dentalPlans = await db.insert(benefitPlans).values([
      {
        companyId: company.id,
        ...
        features: ['Preventive care covered 100%', 'Orthodontics included', 'Large network'],
        effectiveDate: '2024-01-01',
        isActive: true,
      }
    ]).returning();

    const visionPlans = await db.insert(benefitPlans).values([
      {
        companyId: company.id,
        ...
        features: ['Annual eye exams', 'Frames and lenses', 'Contact lens coverage'],
        effectiveDate: '2024-01-01',
        isActive: true,
      }
    ]).returning();
    ...
    console.log(`   - 2 users: ${user1.firstName} ${user1.lastName}, ${user2.firstName} ${user2.lastName}`);
    console.log(`   - ${healthPlans.length + dentalPlans.length + visionPlans.length} benefit plans`);
