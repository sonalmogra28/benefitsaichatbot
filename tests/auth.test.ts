...
describe('Authentication Critical Path Tests', () => {
  ...
  it('should create session on login', async () => {
    ...
    (global.fetch as any).mockResolvedValueOnce({
      ok: true,
      headers: {
        get: (key: string) => key === 'set-cookie' ? 'session=test-token; Path=/; HttpOnly' : null
      },
      json: async () => ({ success: true, user: { email: 'test@example.com' } })
    });
    
    const response = await fetch('http://localhost:3000/api/auth/simple-session', {
      method: 'POST',
  ...
  it('should clear session on logout', async () => {
    ...
    (global.fetch as any).mockResolvedValueOnce({
      ok: true,
      headers: {
        get: (key: string) => key === 'set-cookie' ? 'session=; Path=/; Expires=Thu, 01 Jan 1970 00:00:00 GMT' : null
      },
